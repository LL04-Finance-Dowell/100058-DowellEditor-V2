...
const EmailRightSideBar = () => {
  ...
  const handleSaveRecipientData = () => {
    if (!isChecked) {
      ...
      if (formData.toEmail == '' || formData.toName == '') {
        ...
        const emailDataDiv = targetEmailDiv.querySelector('.emailDataHolder_holder');
        const receiverDataDiv = emailDataDiv.querySelector('.emailRecipientDataHolder_holder');
  ...
  const handleSaveSenderData = (e) => {
    ...
    if (targetEmailDiv) {
      const emailDataDiv = targetEmailDiv.querySelector('.emailDataHolder_holder');
      const senderDataDiv = emailDataDiv.querySelector('.emailSenderDataHolder_holder');
      const data = JSON.stringify(formData)
  ...
  return (
    <>
      {
        actionName === 'template' && <select className='gen_btn_sel' defaultValue='email' onChange={e => setGenSelOpt(e.target.value)}>
